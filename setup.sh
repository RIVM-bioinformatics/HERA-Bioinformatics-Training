...
mamba create -n Consensus_seq longshot medaka bcftools pip python=3.8 pandas==1.3.5 -y >/dev/null 2>&1; source activate Consensus_seq; pip install git+https://github.com/RIVM-bioinformatics/TrueConsense.git@rewrite > /dev/null 2>&1; conda deactivate > /dev/null 2>&1
